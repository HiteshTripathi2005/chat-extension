import { tool } from "ai";
import {z} from "zod";

// Define a tool to get the current time of server
export const timeTool = tool({
    name: "get_current_time",
    description: "Get the current time in a specified timezone (requires IANA timezone string like 'Asia/Kolkata' for India)",
    inputSchema: z.object({
        timezone: z.string().optional().describe("IANA timezone string, e.g., 'Asia/Kolkata'"),
    }),
    execute: (input) => {
        try {
            const { timezone } = input;
            const date = timezone ? new Date().toLocaleString("en-US", { timeZone: timezone }) : new Date().toString();
            return date;
        } catch (error) {
            console.error("Error getting current time:", error);
            throw new Error("Failed to get current time");
        }
    }
})